<script>
	...
	async function handleSubmit() {
		...
		const [url, options] = user.id ? ['api/users/' + user.id, {
			method: 'PUT',
...
<section class="container">
	<article>
		...
		<header style="display: flex; align-items: center; justify-content: space-between;">
			<h2>Users</h2>
			<a href="#form" role="button" on:click={() => {
				handleReset();
				dialog.showModal();
			}}>Add</a>
		</header>
			...
			<tbody>
				{#each Object.entries(users) as [_, { id, name, username, email, role_names }]}
					<tr>
						...
						<td>{role_names}</td>
						<td class="flex">
							<a href="#edit" class="contrast" on:click={() => {
								// @ts-ignore
								user = {...user, ...users[id]};
								dialog.showModal();
							}} role="button">Edit</a>
							<a href="#delete" class="secondary" on:click={()=>{
								handleDelete(id);
...
<dialog bind:this={dialog} style="width: 100%;" on:close={handleReset}>
	<article style="width: 100%;">
		<form method="dialog" bind:this={form} on:submit|preventDefault={handleSubmit} on:reset={handleReset} autocomplete="off">
			...
			<section class="grid">
                ...
                <div>
                    <input type="password" id="password" bind:value={user.password} required={user.id == null} autocomplete="off"/>
